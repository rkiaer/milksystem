    ...
    Private Sub loadData()
        If Page.IsValid Then
            ...
            Try
                ...
                lmsg = String.Concat("Execução da reabertura do romaneio ", Me.txt_nr_romaneio.Text, " realizada com sucesso!")
                ...
                '        lmsg = "O romaneio " + romaneio.id_romaneio.ToString + " ainda não está fechado!"
                '    Else
                '        'se já está fechado
                '        If dsromaneio.Tables(0).Rows(0).Item("id_st_romaneio") = 4 Then
                '            Dim utilitario As New Utilitario
                ...
                '            lmsg = String.Concat("Execução da reabertura do romaneio ", Me.txt_nr_romaneio.Text, " realizada com sucesso!")
                '        Else
                '            lmsg = "O romaneio " + romaneio.id_romaneio.ToString + " não pode ser reaberto no momento porque é um Pré-Romaneio."
                '        End If
                '    End If
                'Else
                ...
                Select Case CInt(ViewState.Item("id_st_romaneio").ToString)
                    Case 4
                        If ViewState.Item("st_leite_pago_referencia").ToString.Equals("S") Then
                            args.IsValid = False
                            lmsg = "O romaneio de número " + txt_nr_romaneio.Text.ToString + " não pode ser reaberto porque já entrou para o cálculo mensal definitivo."
                        Else
                            ...
                            Else
                                args.IsValid = False
                                lmsg = "O romaneio de número " + txt_nr_romaneio.Text.ToString + " não pode ser reaberto porque já foi realizado Cálculo Mensal Definitivo após sua finalização."

                            End If
                        End If
                    Case Is < 4
                        args.IsValid = False
                        lmsg = "O romaneio " + txt_nr_romaneio.Text.ToString + " ainda não está fechado!"
                    Case Is > 4
                        args.IsValid = False
                        lmsg = "O romaneio " + txt_nr_romaneio.Text.ToString + " não pode ser reaberto no momento porque é um Pré-Romaneio."
                End Select
