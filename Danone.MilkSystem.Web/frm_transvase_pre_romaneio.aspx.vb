        ...
        Try
            ...
            Else
                If ViewState.Item("gridPreRomaneioTemLinhas") Is Nothing Then
                    ...
                    gridPreRomaneio.Rows(0).Cells(0).Text = "Não existe nenhum Pré-Romaneio associado a este Transvase!"
                    ...
                    gridTransitPoint.Rows(0).Cells(0).Text = "Não existe nenhum volume de pré-romaneio associado a esse Transvase!"
    ...
    Private Sub retirarPreRomaneio(ByVal id_pre_romaneio As Integer)
        ...
        Try
            ...
            If transvaseup.getTransvaseUPByFilters.Tables(0).Rows.Count = 0 Then
                ...
                messageControl.Alert("Pré-Romaneio retirado do Transvase com sucesso!")
            Else
                messageControl.Alert("Pré-Romaneio não foi retirado do Transvase. Entre em contato com administrador do sistema.")
        ...
        Try
            ...
            Select Case e.CommandName.ToString().ToLower()
                Case "retirarup"
                    ...
                    messageControl.Alert("O volume de leite do Produtor " & lprodutor.Text & ", propriedade/UP " & lpropriedade.Text & " do Pré-Romaneio " & ViewState.Item("id_pre_romaneio").ToString & " foi retirado do Transvase com sucesso.")
    ...
    Private Sub loadGridPreRomaneio()
        Try
            ...
            Else
                ...
                gridPreRomaneio.Rows(0).Cells.Add(New TableCell())
                gridPreRomaneio.Rows(0).Attributes.CssStyle.Add("text-align", "center")
                gridPreRomaneio.Rows(0).Cells(0).Text = "Não existe nenhum Pré-Romaneio informado para o Transvase!"
    ...
    Private Sub loadGridtransvaseProdutores()
        Try
            ...
            Else
                ...
                gridTransitPoint.Rows(0).Cells.Add(New TableCell())
                gridTransitPoint.Rows(0).Attributes.CssStyle.Add("text-align", "center")
                gridTransitPoint.Rows(0).Cells(0).Text = "Não existe nenhum volume de Pré-Romaneio associado ao Transvase!"
                gridTransitPoint.Rows(0).Cells(0).ColumnSpan = 16
    ...
    Protected Sub cv_pre_romaneio_ServerValidate(ByVal source As Object, ByVal args As System.Web.UI.WebControls.ServerValidateEventArgs) Handles cv_pre_romaneio.ServerValidate
        Try
            ...
            If tppreromaneio.getTransvasePreRomaneioByFilters().Tables(0).Rows.Count > 0 Then
                args.IsValid = False
                lmsg = "O número informado no campo 'Pré-Romaneio' não pode ser incluído porque já existe no Transvase."
            End If
                ...
                If (dsromaneio.Tables(0).Rows.Count > 0) Then
                    With dsromaneio.Tables(0).Rows(0)
                        ...
                        If CInt(.Item("id_st_romaneio").ToString) < 7 Then
                            args.IsValid = False
                            lmsg = "O número informado no campo 'Pré-Romaneio' não pode ser incluído porque não corresponde a um Pré-Romaneio de Transvase."
                        Else
                            ...
                            If CInt(ViewState.Item("id_estabelecimento").ToString) = CInt(.Item("id_estabelecimento").ToString) Then
                                ...
                                If Not (.Item("id_st_romaneio").ToString.Equals("14") OrElse .Item("id_st_romaneio").ToString.Equals("16")) Then
                                    args.IsValid = False
                                    lmsg = "O Pré-Romaneio informado não pode ser incluído porque sua situação é '" & .Item("nm_st_romaneio").ToString & "'. Para inclusão no Transvase deve estar como disponível ou em uso parcial."
                                End If
                            Else
                                args.IsValid = False
                                lmsg = "O Pré-Romaneio informado não pode ser incluído porque não foi cadastrado no mesmo Estabelecimento do Transvase " & lbl_id_transit_point.Text & "."
        ...
        Try
            ...
            Else
                ...
                If (CLng(transvasecomp.nr_total_litros) + CLng(lbl_volume_selecionado.Text)) > CLng(transvasecomp.nr_volume_maximo) Then
                    args.IsValid = False
                    lmsg = "O volume total selecionado não pode ultrapassar a capacidade máxima do compartimento do Transvase que é de " & FormatNumber(transvasecomp.nr_volume_maximo, 0).ToString & "."
                End If
                    ...
                    For Each row In gridDetalhePreRomaneio.Rows
                        ...
                        If chk_selecionar.Checked = True Then
                            ...
                            If CDec(txt_nr_volume_tp.Text) > CDec(nr_volume_saldo_pre_romaneio.Text) Then
                                args.IsValid = False
                                lmsg = String.Concat("O volume informado para o Transvase não poder ser maior que o Volume Saldo do pré-romaneio, para a propriedade ", CType(row.Cells(4), DataControlFieldCell).Text, ", Placa/Comp ", CType(row.Cells(1), DataControlFieldCell).Text, "/", CType(row.Cells(2), DataControlFieldCell).Text, ".")
                            End If
                        End If
                    Next
                End If
            ...
            Try
                ...
                If Not ViewState.Item("id_pre_romaneio") Is Nothing AndAlso CInt(ViewState.Item("id_pre_romaneio")) > 0 Then
                    ...
                    If CInt(e.Row.Cells(7).Text) = 0 Then
                        ...
                        btn_delete.ToolTip = "Não há volume desta propriedade adicionado no Transvase."
                    Else
