...
Partial Class frm_romaneio_cooperativa_abertura
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            usuariolog.id_menu_item = 18
            usuariolog.insertUsuarioLog()
            'FRAN 08/12/2020  incluir log 

            'fran 03/01/2024 i logistica
            If Not (Request("id_romaneio_cooperativa") Is Nothing) Then
                ViewState.Item("id_romaneio_cooperativa") = Request("id_romaneio_cooperativa")
            Else
                ViewState.Item("id_romaneio_cooperativa") = 0
            End If
            'fran 03/01/2024 f logistica
    ...
    Private Sub loadDetails()
        ...
        Try
            ...
            'fran 03/01/2024 i logistica
            If CInt(ViewState.Item("id_romaneio_cooperativa")) > 0 Then
    ...
    End Sub
    Private Sub loadDataNotas()

        Try

            Dim romaneio As New Romaneio
            romaneio.id_romaneio = Convert.ToInt32(ViewState.Item("id_romaneio_cooperativa"))

            Dim dsromaneio As DataSet = romaneio.getRomaneioCooperativaNotas

            tr_romaneio.Visible = True
            lbl_romaneio_cooperativa.Text = ViewState.Item("id_romaneio_cooperativa").ToString
            ...
            With dsromaneio.Tables(0).Rows(0)
                ...
                Select Case CInt(ViewState.Item("id_estado_cooperativa"))
                    Case 0

                    Case 11
                        cbo_natureza_operacao.SelectedValue = 2
                    Case Else
                        cbo_natureza_operacao.SelectedValue = 4
                End Select
                ...
                Me.txt_cd_transportador.Text = .Item("cd_transportador").ToString

                loadMotoristaByTransportador(.Item("id_transportador").ToString)
                txt_nr_nota_fiscal.Text = .Item("nr_nota_fiscal").ToString
                txt_serie_nota_fiscal.Text = .Item("nr_serie_nf").ToString
                txt_dt_saida_nota.Text = Format(DateTime.Parse(.Item("dt_emissao_nf").ToString), "dd/MM/yyyy").ToString
                txt_nr_peso_liquido_nota.Text = (CDec(.Item("nr_peso_liquido_nota"))).ToString("######")
                txt_nr_valor_nota_fiscal.Text = (CDec(.Item("nr_valor_nf"))).ToString("######.00")
                txt_ds_chave_nfe.Text = .Item("ds_chave_nf").ToString
                txt_nr_po.Text = .Item("nr_po_cooperativa").ToString
                cbo_tipo_nota_fiscal.SelectedValue = 1 'nota de venda
                txt_cd_cfop.Text = .Item("cd_cfop").ToString
                If txt_cd_cfop.Text.Equals("5120") OrElse txt_cd_cfop.Text.Equals("6120") Then
                    lbl_informativo_remessa.Text = "*A nota de leite informada é uma NOTA FISCAL DE REMESSA. A nota fiscal origem deve ser anexada após salvar o Romaneio ou em Consulta de Romaneio."
                    lbl_informativo_remessa.Visible = True
                Else
                    lbl_informativo_remessa.Text = String.Empty
                    lbl_informativo_remessa.Visible = False
                End If

                cbo_id_item.SelectedValue = 1 'default leite in natura
                If .Item("nr_modelo_frete").ToString = "1" Then 'se frete nota fob
                    cbo_tipo_frete.SelectedValue = 2 'fob
                    cbo_tipo_frete.Enabled = False
                    txt_nr_cte.Text = .Item("nr_nota_fiscal_cte").ToString
                    txt_serie_cte.Text = .Item("nr_serie_cte").ToString
                    txt_nr_valor_cte.Text = FormatNumber(CDec(.Item("nr_valor_nf_cte").ToString), 2)
                    txt_ds_chave_cte.Text = .Item("ds_chave_cte").ToString
                    cbo_CST_cte.SelectedValue = .Item("cd_cst_cte").ToString
                    If Not IsDBNull(.Item("nr_valor_icms_cte")) AndAlso CDec(.Item("nr_valor_icms_cte").ToString) > 0 Then
                        txt_valor_icms_cte.Text = FormatNumber(CDec(.Item("nr_valor_icms_cte").ToString), 2)
                    Else
                        txt_valor_icms_cte.Text = String.Empty
                    End If
                    If Not IsDBNull(.Item("nr_base_icms_cte")) AndAlso CDec(.Item("nr_base_icms_cte").ToString) > 0 Then
                        txt_base_icms_cte.Text = FormatNumber(CDec(.Item("nr_base_icms_cte").ToString), 2)
                    Else
                        txt_base_icms_cte.Text = String.Empty
                    End If

                    'se nao é isento de icms e nao veio valor de icms deixa salvar
                    If Not cbo_CST_cte.SelectedValue.Equals("40") AndAlso txt_valor_icms_cte.Text.Equals(String.Empty) Then
                        txt_valor_icms_cte.Enabled = True
                        txt_base_icms_cte.Enabled = True
                        rf_icms_cte.Enabled = True
                        rf_base_icms_cte.Enabled = True
                    End If
                    If Not IsDBNull(.Item("dt_emissao_completa_cte")) Then
                        txt_dt_emissao_cte.Text = Format(DateTime.Parse(.Item("dt_emissao_completa_cte").ToString), "dd/MM/yyyy").ToString
                        txt_hr_emissao_cte.Text = Format(DateTime.Parse(.Item("dt_emissao_completa_cte")), "HH")
                        txt_mm_emissao_cte.Text = Format(DateTime.Parse(.Item("dt_emissao_completa_cte")), "mm")
                        txt_ss_emissao_cte.Text = Format(DateTime.Parse(.Item("dt_emissao_completa_cte")), "ss")
                    Else
                        txt_dt_emissao_cte.Enabled = True
                        rf_dt_emissao_cte.Enabled = True
                        txt_hr_emissao_cte.Enabled = True
                        txt_mm_emissao_cte.Enabled = True
                        txt_ss_emissao_cte.Enabled = True

                    End If

                    txt_cd_cfop_cte.Text = .Item("cd_cfop_cte").ToString

                    If Not IsDBNull(.Item("ds_protocolo_cte")) AndAlso Not .Item("ds_protocolo_cte").ToString.Equals(String.Empty) Then
                        txt_nr_protocolo_cte.Text = .Item("ds_protocolo_cte").ToString
                    Else
                        txt_nr_protocolo_cte.Enabled = True
                        rf_nr_protocolo_cte.Enabled = True
                    End If

                    If Not IsDBNull(.Item("cd_uf_origem_cte")) AndAlso Not .Item("cd_uf_origem_cte").ToString.Equals(String.Empty) Then
                        txt_uf_origem_cte.Text = .Item("cd_uf_origem_cte").ToString
                    Else
                        txt_uf_origem_cte.Enabled = True
                        rf_uf_origem_cte.Enabled = True
                    End If
                    If Not IsDBNull(.Item("cd_uf_destino_cte")) AndAlso Not .Item("cd_uf_destino_cte").ToString.Equals(String.Empty) Then
                        txt_uf_destino_cte.Text = .Item("cd_uf_destino_cte").ToString
                    Else
                        txt_uf_destino_cte.Enabled = True
                        rf_uf_destino_cte.Enabled = True
                    End If
                    If Not IsDBNull(.Item("cd_ibge_origem_cte")) AndAlso Not .Item("cd_ibge_origem_cte").ToString.Equals(String.Empty) Then
                        txt_ibge_origem_cte.Text = .Item("cd_ibge_origem_cte").ToString
                    Else
                        txt_ibge_origem_cte.Enabled = True
                        rf_ibge_origem_cte.Enabled = True
                    End If
                    If Not IsDBNull(.Item("cd_ibge_destino_cte")) AndAlso Not .Item("cd_ibge_destino_cte").ToString.Equals(String.Empty) Then
                        txt_ibge_destino_cte.Text = .Item("cd_ibge_destino_cte").ToString
                    Else
                        txt_ibge_destino_cte.Enabled = True
                        rf_ibge_destino_cte.Enabled = True
                    End If

                Else
                    'se frete nao tem frete danone, nao deixa selecionar fob, apenas cif e outros
                    'cbo_tipo_frete.Items.FindByValue("2").Enabled = False 'fran 07/02/2024
                    cbo_tipo_frete.SelectedValue = 1 'cif
                    cbo_tipo_frete.Enabled = False

                End If
            End With

            'desabilitar campos da nota ... nao permite alteracoes
            ...
            txt_serie_nota_fiscal.Enabled = False
            txt_dt_saida_nota.Enabled = False
            txt_nr_peso_liquido_nota.Enabled = False
            txt_nr_valor_nota_fiscal.Enabled = False
            txt_ds_chave_nfe.Enabled = False
            If txt_nr_po.Text.Equals(String.Empty) Then
                txt_nr_po.Enabled = True
            Else
                txt_nr_po.Enabled = False
            End If
            cbo_tipo_nota_fiscal.Enabled = False
            txt_cd_cfop.Enabled = False
    ...
    Private Sub updateData()
        If Page.IsValid Then
            ...
            Try
                ...
                'fran 13/06/2024 i para evitar mais de um clique
                Dim lbupdateromaneio As Boolean = True

                If CInt(ViewState.Item("id_romaneio_cooperativa")) > 0 Then 'se veio de baertura de nota automatica
                    romaneio.id_romaneio = Convert.ToInt32(ViewState.Item("id_romaneio_cooperativa"))
                    Dim dsromaneio As DataSet = romaneio.getRomaneioCooperativaNotas
                    If dsromaneio.Tables(0).Rows.Count > 0 Then
                        If CInt(dsromaneio.Tables(0).Rows(0).Item("id_st_romaneio").ToString) > 1 Then
                            lbupdateromaneio = False
                        End If
                    End If
                ...
                If lbupdateromaneio = True Then
                    ...
                    romaneio.nr_po_cooperativa = txt_nr_po.Text.ToString 'fran 03/01/2024
                    ...
                    romaneio.cd_cfop = txt_cd_cfop.Text
                    ...
                    If romaneio.id_frete_nf.Equals(2) Then 'se FOB
                        ...
                        romaneio.nr_serie_cte = txt_serie_cte.Text
                        romaneio.ds_chave_cte = txt_ds_chave_cte.Text.Trim
                        romaneio.cd_cst_cte = cbo_CST_cte.SelectedValue
                        If txt_valor_icms_cte.Text.Equals(String.Empty) Then
                            romaneio.nr_valor_icms_cte = 0
                        Else
                            romaneio.nr_valor_icms_cte = txt_valor_icms_cte.Text
                        End If
                        If txt_base_icms_cte.Text.Equals(String.Empty) Then
                            romaneio.nr_base_icms_cte = 0
                        Else
                            romaneio.nr_base_icms_cte = Convert.ToDecimal(txt_base_icms_cte.Text)
                        End If
                        romaneio.dt_emissao_completa_cte = txt_dt_emissao_cte.Text & " " & txt_hr_emissao_cte.Text & ":" & txt_mm_emissao_cte.Text & ":" & txt_ss_emissao_cte.Text
                        romaneio.cd_cfop_cte = txt_cd_cfop_cte.Text
                        romaneio.ds_protocolo_cte = txt_nr_protocolo_cte.Text
                        romaneio.cd_uf_origem_cte = txt_uf_origem_cte.Text
                        romaneio.cd_uf_destino_cte = txt_uf_destino_cte.Text
                        romaneio.cd_ibge_origem_cte = txt_ibge_origem_cte.Text
                        romaneio.cd_ibge_destino_cte = txt_ibge_destino_cte.Text

                    Else
                        romaneio.nr_cte = 0
                        romaneio.nr_valor_cte = 0
                        romaneio.nr_serie_cte = String.Empty
                        romaneio.ds_chave_cte = String.Empty
                        romaneio.cd_cst_cte = String.Empty
                        romaneio.nr_valor_icms_cte = 0
                        romaneio.nr_base_icms_cte = 0
                        romaneio.dt_emissao_completa_cte = String.Empty
                        romaneio.cd_cfop_cte = String.Empty
                        romaneio.ds_protocolo_cte = String.Empty
                        romaneio.cd_uf_origem_cte = String.Empty
                        romaneio.cd_uf_destino_cte = String.Empty
                        romaneio.cd_ibge_origem_cte = String.Empty
                        romaneio.cd_ibge_destino_cte = String.Empty
                    ...
                    If CInt(ViewState.Item("id_romaneio_cooperativa")) > 0 Then
                        romaneio.id_romaneio = Convert.ToInt32(ViewState.Item("id_romaneio_cooperativa"))
                        romaneio.abrirRomaneioCooperativa()
                    Else
    ...
    Protected Sub cbo_tipo_frete_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_tipo_frete.SelectedIndexChanged
        Try
            ...
            If cbo_tipo_frete.SelectedValue.Equals("2") Then 'SE FOB  informa cte
                ...
                txt_nr_protocolo_cte.Text = String.Empty
                txt_dt_emissao_cte.Text = String.Empty
                txt_hr_emissao_cte.Text = String.Empty
                txt_mm_emissao_cte.Text = String.Empty
                txt_ss_emissao_cte.Text = String.Empty
                txt_base_icms_cte.Enabled = True
                txt_uf_origem_cte.Enabled = True
                txt_uf_destino_cte.Enabled = True
                txt_ibge_origem_cte.Enabled = True
                txt_ibge_destino_cte.Enabled = True
                txt_cd_cfop_cte.Enabled = True
                txt_nr_protocolo_cte.Enabled = True
                txt_dt_emissao_cte.Enabled = True
                txt_hr_emissao_cte.Enabled = True
                txt_mm_emissao_cte.Enabled = True
            ...
            Else
                ...
                txt_valor_icms_cte.Enabled = False
                ...
                txt_base_icms_cte.Enabled = False
                txt_uf_origem_cte.Enabled = False
                txt_uf_destino_cte.Enabled = False
                txt_ibge_origem_cte.Enabled = False
                txt_ibge_destino_cte.Enabled = False
                txt_cd_cfop_cte.Enabled = False
                txt_nr_protocolo_cte.Enabled = False
                txt_dt_emissao_cte.Enabled = False
                txt_hr_emissao_cte.Enabled = False
                txt_mm_emissao_cte.Enabled = False
                txt_ss_emissao_cte.Enabled = False
                ...
                rf_cd_cfop_cte.Enabled = False
                rf_nr_protocolo_cte.Enabled = False
    ...
    Protected Sub txt_cd_cfop_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_cd_cfop.TextChanged
        If txt_cd_cfop.Text.Equals("5120") OrElse txt_cd_cfop.Text.Equals("6120") Then
            lbl_informativo_remessa.Text = "*A nota de leite informada é uma NOTA FISCAL DE REMESSA. A nota fiscal origem deve ser anexada após salvar o Romaneio ou em Consulta de Romaneio."
            lbl_informativo_remessa.Visible = True
        Else
