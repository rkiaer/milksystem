...
' 10/05/2012 - Projeto Themis - chamado 1547 (nova aplicação para registrar pesagem da balança) - i

Partial Class lst_romaneio_pesagem_balanca
    ...
    Protected Sub btn_limparFiltros_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btn_limparFiltros.Click
        ...
        Response.Redirect("lst_romaneio_pesagem_balanca.aspx?st_incluirlog=N")

    End Sub

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        customPage = MenuTools.getInitialContext(HttpContext.Current, "lst_romaneio_pesagem_balanca.aspx")
    ...
    Private Sub loadFilters()
        ...
        If Not (customPage.getFilterValue("lst_rompesbal", txt_id_romaneio.ID).Equals(String.Empty)) Then
            hasFilters = True
            ViewState.Item("id_romaneio") = customPage.getFilterValue("lst_rompesbal", txt_id_romaneio.ID)
            txt_id_romaneio.Text = ViewState.Item("id_romaneio").ToString()
        Else
            ViewState.Item("id_romaneio") = String.Empty
        End If

        If Not (customPage.getFilterValue("lst_rompesbal", hf_id_pessoa.ID).Equals(String.Empty)) Then
            hasFilters = True
            ViewState.Item("id_transportador") = customPage.getFilterValue("lst_rompesbal", hf_id_pessoa.ID)
        ...
        If Not (customPage.getFilterValue("lst_rompesbal", txt_placa.ID).Equals(String.Empty)) Then
            hasFilters = True
            ViewState.Item("ds_placa") = customPage.getFilterValue("lst_rompesbal", txt_placa.ID)
        ...
        ' 22/01/2009 - Rls15 - i
        If Not (customPage.getFilterValue("lst_rompesbal", txt_nm_linha.ID).Equals(String.Empty)) Then
            hasFilters = True
            ViewState.Item("nm_linha") = customPage.getFilterValue("lst_rompesbal", txt_nm_linha.ID)
            txt_nm_linha.Text = ViewState.Item("nm_linha").ToString()
        Else
            ViewState.Item("nm_linha") = String.Empty
        End If
        If Not (customPage.getFilterValue("lst_rompesbal", txt_dt_transmissao_ini.ID).Equals(String.Empty)) Then
            hasFilters = True
            ViewState.Item("dt_transmissao_ini") = customPage.getFilterValue("lst_rompesbal", txt_dt_transmissao_ini.ID)
            txt_dt_transmissao_ini.Text = ViewState.Item("dt_transmissao_ini").ToString()
        Else
            ViewState.Item("dt_transmissao_ini") = String.Empty
        End If
        If Not (customPage.getFilterValue("lst_rompesbal", txt_dt_transmissao_fim.ID).Equals(String.Empty)) Then
            hasFilters = True
            ViewState.Item("dt_transmissao_fim") = customPage.getFilterValue("lst_rompesbal", txt_dt_transmissao_fim.ID)
        ...
        If Not (customPage.getFilterValue("lst_rompesbal", "PageIndex").Equals(String.Empty)) Then
            ViewState.Item("PageIndex") = customPage.getFilterValue("lst_rompesbal", "PageIndex").ToString()
        ...
        If (hasFilters) Then
            loadData()
            customPage.clearFilters("lst_rompesbal")
        Else
    ...
    Private Sub loadData()
        ...
        Try
            ...
            Dim dsromaneio As DataSet = romaneio.getRomaneioPesagemBalanca()  ' chamado 1547 - 10/05/2012 (procedure ms_getRomaneioPesagemBalanca para trazer Romaneios Abertos, Em Análises e Fechados com registro de exportação de batch igual 1-Pendente)
    ...
    Private Sub saveFilters()
        ...
        Try
            ...
            customPage.setFilter("lst_rompesbal", txt_id_romaneio.ID, ViewState.Item("id_romaneio").ToString)
            customPage.setFilter("lst_rompesbal", hf_id_pessoa.ID, ViewState.Item("id_transportador").ToString)
            customPage.setFilter("lst_rompesbal", txt_placa.ID, ViewState.Item("ds_placa").ToString)
            customPage.setFilter("lst_rompesbal", txt_nm_linha.ID, ViewState.Item("nm_linha").ToString)
            customPage.setFilter("lst_rompesbal", txt_dt_transmissao_ini.ID, ViewState.Item("dt_transmissao_ini").ToString)
            customPage.setFilter("lst_rompesbal", txt_dt_transmissao_fim.ID, ViewState.Item("dt_transmissao_fim").ToString)
            customPage.setFilter("lst_rompesbal", "PageIndex", gridResults.PageIndex.ToString())
    ...
    Protected Sub gridResults_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gridResults.RowCommand
        ...
        Select Case e.CommandName.ToString().ToLower()
            ...
            Case "selecionar"
                ...
                Response.Redirect("frm_romaneio_pesagem_balanca.aspx?id_romaneio=" + e.CommandArgument.ToString)
    ...
    End Sub
    ' 10/05/2012 - Projeto Themis - chamado 1547 (nova aplicação para registrar pesagem da balança) - f
